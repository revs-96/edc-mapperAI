import os
from parsers import sponsor_b_parser
import pandas as pd
...
MODEL_SAVE_PATH = "models/model_sponsor_b_v1.pkl"
os.makedirs("models", exist_ok=True)

def train_sponsor_b(odm_path, viewmap_path, model_save_path):

    odm_mappings = sponsor_b_parser.parse_odm_file(odm_path)
    view_mappings = sponsor_b_parser.parse_view_mapping_file(viewmap_path)

    train_df = build_training_dataset(odm_mappings, view_mappings,
                                      study_event_col="StudyEventOIDSponsB",
                                      item_oid_col="ItemOIDSponsB",
                                      visit_col="IMPACTVisitIDSponsB",
                                      attr_col="IMPACTAttributeIDSponsB")

    X = train_df[[ "StudyEventOIDSponsB", "ItemOIDSponsB" ]].astype(str)
    y_visit = train_df["IMPACTVisitIDSponsB"].astype(str)
    y_attr = train_df["IMPACTAttributeIDSponsB"].astype(str)
    ...
    X_encoded = pd.DataFrame({
        "StudyEventOID": le_studyevent.fit_transform(X["StudyEventOIDSponsB"]),
        "ItemOID": le_item.fit_transform(X["ItemOIDSponsB"]),
    })
    ...
    with open(model_save_path, "wb") as f:
        pickle.dump(model, f)
    print(f"Saved Sponsor B model to {model_save_path}")
